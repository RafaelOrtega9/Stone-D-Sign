(function () {
  ...
  function mountApplications() {
    ...
    target.innerHTML = data.applications.map(function (a, i) {
      var shape = ["ph--wide", "ph--tall", "ph--square", "ph--wide", "ph--tall"][i % 5];
      var media = a.photo
        ? '<div class="app-media ' + shape + '">' +
            '<img src="' + escHTML(a.photo) + '" alt="' + escHTML(a.alt || a.title) + '" loading="lazy" decoding="async">' +
          '</div>'
  ...
  function setupContactForm() {
    ...
    form.addEventListener("submit", function (e) {
      ...
      fetch("submit-contact.php", { method: "POST", body: body })
        .then(function (res) { return res.json().catch(function () { return { success: false, error: "bad_response" }; }); })
        .then(function (json) {
          submitBtn.disabled = false;
          form.classList.remove("is-sending");
          if (json && json.success) {
            var firstName = json.firstName || "there";
            ...
            if (errorEl) errorEl.textContent = "Something went wrong sending your request — please call or email us directly.";
          }
        })
        .catch(function () {
          submitBtn.disabled = false;
